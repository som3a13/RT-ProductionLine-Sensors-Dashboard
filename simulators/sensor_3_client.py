"""
Sensor 3 Client - Flow Rate Sensor (TCP Client)
...
class Sensor3Client:
    """TCP client for Sensor 3 (Flow Rate Sensor)"""
    
    def __init__(self, server_host="localhost", server_port=5000):
        """
        Initialize Sensor 3 TCP client
        ...
        self.sensor_id = 3
        self.sensor_name = "Flow Rate Sensor 1"
        self.low_limit = 10.0
    ...
    def create_working_frame(self, value: float) -> bytes:
        """
        Create working frame for Sensor 3
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Sensor 3 (Flow Rate) TCP Client')
    ...
    client = Sensor3Client(server_host=args.server_host, server_port=args.server_port)
